...
void concatenateImageChannel(float *r, float *g, float *b, int w, int h, QImage &image)
{
    ...
    for (int j=0; j<h; j++) {
        for (int i=0; i<w; i++) {
            QRgb pixel = qRgb((qintptr)r, (qintptr)g, (qintptr)b);
...
//分离图像的三通道后，调用rgb2ycrcb()函数完成图像RGB—>ycrcb的转换
...
QImage calculateNegative(QImage &image, ImageChannel channel)
{
    ...
    for (int j=0; j<height; j++) {
        for (int i=0; i<width; i++) {
            ...
            QRgb newPixel;
            //选取负片的通道
            //Y通道：RGB所有通道均用最大值255减去每个像素点的的值
            switch (channel) {
                case ImageChannel::Y:
                    newPixel = qRgb(255-r, 255-g, 255-b);
                    break;

                    //R通道：只有R通道用最大值减去像素点R值
                case ImageChannel::R:
                    newPixel = qRgb(255-r, g, b);
                    break;
                    //G通道：只有G通道用最大值减去像素点G值
                case ImageChannel::G:
                    newPixel = qRgb(r, 255-g, b);
                    break;
                    //B通道：只有B通道用最大值减去像素点B值
                case ImageChannel::B:
...
//直方图均衡化（改变各像素灰度，增大灰度值动态范围，增强图像对比度）
